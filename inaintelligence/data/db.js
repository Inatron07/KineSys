...
async function getCfPersonDailyLedger(accountId, personId, from, to) {
  ...
  const { rows } = await pool.query(
    `SELECT d::date AS day,
       ...
       STRING_AGG(DISTINCT CASE WHEN t.type='bill' AND t.counterparty IS NOT NULL AND t.counterparty <> '' THEN t.counterparty END, ', ') AS bill_vendor_name,
  ...
  const days = rows.map((r) => {
    ...
    return {
      ...
      billAmount,
      billVendorName: r.bill_vendor_name || '',
      noBillAmount,
